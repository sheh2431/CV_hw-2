...
def PCA(input_data):
    ...
    k_eigen_vector = eigen_vector[:, eigSortIndex[0:k]] # k * 2576
    print("eigen: ", eigen_vector)
    #coutn = 0
    #while (count < k):
    ...
    #            count+=1
    #            break
    eigen_faces = np.array(np.dot(k_eigen_vector.T, diff)) # 280 * 2576
    print("PRINT:", eigen_faces.shape)
...
def testing(Test_input, mean_face, eigen_faces, eigenvector):
    print("eigen_faces: ", eigen_faces.shape)
    h, w = Test_input.shape
    print(h, w)

    test_diff = np.subtract(Test_input, np.tile(np.mean(Test_input), (h, 1)))
    print(test_diff.shape)
    print(np.tile(mean_face, (h, 1)).shape)
    test_project = np.dot(eigen_faces.T, test_diff).T + np.tile(np.mean(Test_input), (h, 1)).T
    plt.imshow(test_project[0,:].reshape(origin_h, origin_w), cmap='gray')
    #plt.title("n=" + str(k) + "\nMSE: "+str(MSE))
    plt.show()
